...
class HEALPixSampler(BaseSampler):
    ...
    def sample(self, n_sample, *args, **kwargs):
        ...
        if self.pipeline is not None and self.cfg.check_density:
            ...
            result = sample_poses_HEALPix(n_side=2)
...
class SphericalSampler(BaseSampler):
    ...
    def sample(self, n_sample, *args, **kwargs):
        ...
        if self.pipeline is not None and self.cfg.check_density:
            ...
            result = sample_poses_spherical(n_sample)
...
def sample_poses_HEALPix(n_side=2):
    poses = generate_HEALPix_viewpoints(n_side=n_side)
    return poses
