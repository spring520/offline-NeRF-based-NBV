...
class NBVDataParser(DataParser):
    ...
    def _generate_dataparser_outputs(self, split="train", num_images: int = 4):
        ...
        cameras = Cameras(
            fx=float(fl_x), # 256
            fy=float(fl_y), # 256
            cx=float(meta.get("cx", 0.5 * w)), # 256
            cy=float(meta.get("cy", 0.5 * h)), # 256
            distortion_params=distortion_params, # 6个0
            height=int(h), # 512
            width=int(w), # 512
            camera_to_worlds=camera_to_world, # 3x4 
            camera_type=camera_type, # camera_type = CameraType.PERSPECTIVE from nerfstudio.cameras.cameras import Cameras, CameraType
        )
